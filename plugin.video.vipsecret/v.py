...
addon = xbmcaddon.Addon('plugin.video.vipsecret')
...
#source_file
#SOURCE: [{"url": "https://goo.gl/GWC619", "fanart": ".\\fanart.jpg", "title": "VipSecret"}]
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[VipSecret-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(VipSecret ,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(VipSecret ,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(VipSecret ,New source added.,5000,"+icon+")")
        if not url is None:
...
#            ln+='</item>'
    ...
    xbmc.executebuiltin("XBMC.Notification(VipSecret ,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
