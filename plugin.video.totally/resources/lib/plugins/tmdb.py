"""
    tmdb.py --- totally Plugin for accessing tmdb data
...
from resources.lib.util.xml import totallyItem, totallyList, display_list
...
def tmdb(url):
    ...
    totallylist = totallyList(xml)
    display_list(totallylist.get_list(), __builtin__.content_type)
...
def tmdb_tv_show(url):
    ...
    if not xml:
        ...
        save_to_db(xml, url)
    totallylist = totallyList(xml)
    display_list(totallylist.get_list(), totallylist.get_content_type())
