...
def totally_get_tag_content(collection, tag, default):
    try:
...
def totally_list(content):
    ...
    vip = totally_get_tag_content(info, 'poster', '0')
    image = totally_get_tag_content(info, 'thumbnail', '0')
    fanart = totally_get_tag_content(info, 'fanart', '0')
    ...
    for item in items:
        try:
            ...
            date = totally_get_tag_content(item, 'date', '')
            if re.search(r'\d+', date):
                name += ' [COLOR red] Updated %s[/COLOR]' % date
            meta = totally_get_tag_content(item, 'meta', '0')
            url = totally_get_tag_content(item, 'link', '0')
            ...
            image2 = totally_get_tag_content(item, 'thumbnail', image)
            if not str(image2).lower().startswith('http'):
                image2 = '0'

            fanart2 = totally_get_tag_content(item, 'fanart', fanart)
            if not str(fanart2).lower().startswith('http'):
                fanart2 = '0'

            content = totally_get_tag_content(meta, 'content', '0')

            if content in ['episode', 'season']:
                continue

            imdb = totally_get_tag_content(meta, 'imdb', '0')
            tvdb = totally_get_tag_content(meta, 'tvdb', '0')
            tvshowtitle = totally_get_tag_content(meta, 'tvshowtitle', '0')
            title = totally_get_tag_content(meta, 'title', '0')
            if title == '0' and not tvshowtitle == '0':
                title = tvshowtitle

            year = totally_get_tag_content(meta, 'year', '0')
            premiered = totally_get_tag_content(meta, 'premiered', '0')
            season = totally_get_tag_content(meta, 'season', '0')
            episode = totally_get_tag_content(meta, 'episode', '0')
...
def main():
    ...
    sections = [
        {'name': 'totally', # can change totally to whatever you want.  This wil name the output xml totally.xml
         'url': "http://yoururlforthesectionyouwant.xml", # your section xml goes here
         'poster': 'totally'}, # can change totally to whatever you want. This will be displayed as the poster name on totally.xml and
                           # also in the actual search in your addon in kodi eg totally - The Avengers
    ]
