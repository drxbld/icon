...
DEBUGPREFIX = '[COLOR red][ Stefano DEBUG ][/COLOR]' # Using color coding, for color formatted log viewers like Assassin's Tools
...
def log(msg, level=LOGNOTICE):
    ...
    try:
        ...
        if not control.setting('debug.location') == '0':
            log_file = os.path.join(LOGPATH, 'stefano.log')
