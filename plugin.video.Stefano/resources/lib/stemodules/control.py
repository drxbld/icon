...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.Stefano.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.Stefano.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.Stefano.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.Stefano.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.Stefano.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.Stefano.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
