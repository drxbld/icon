...
from shows import link
from resources.lib.stemodules import cache
from resources.lib.stemodules import metacache
...
from resources.lib.stemodules import trailer
from resources.lib.stemodules import workers
from resources.lib.stemodules import youtube
...
class indexer:
    ...
    def getRequestUrl(url):
        if url.startswith("https://stefanoaddon.info/") and (url.endswith(".xml") or "page=" in url):
            if url.endswith(".xml"):
                fname = url.split("/")[-1]
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.Stefano_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            result = f.read()
            f.close()
            self.list = self.Stefano_list('', result=result)
    ...
    def search(self, url):
        try:
            mark = False
            if (url == None or url == ''):
                self.list = [{'name': control.lang(30702).encode("utf-8"), 'action': 'addSearch'}]
                self.list += [{'name': control.lang(30703).encode("utf-8"), 'action': 'delSearch'}]
            else:
                if '|SECTION|' in url: mark = url.split('|SECTION|')[0]
                self.list = [{'name': control.lang(30702).encode("utf-8"), 'url': url, 'action': 'addSearch'}]
                self.list += [{'name': control.lang(30703).encode("utf-8"), 'action': 'delSearch'}]
    ...
    def addSearch(self, url):
        ...
        links = client.request(self.getServerUrl(link))
        ...
        self.list = []
        threads = []
        for link in links: threads.append(workers.Thread(self.Stefano_list, link, None, True))
        ...
        for i in self.list:
            try:
                name = ''
                if not i['vip'] in ['Stefano TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                name += i['name']
                i.update({'name': name})
            except:
                pass
    ...
    def Stefano_list(self, url, result=None, search=False):
        try:
            if result is None:
                if search:
                ...
                result = cache.get(client.request, 0, url, True, True, False, None, None, headers)
        ...
        for item in items:
            ...
            try:
                ...
                item = item.replace('\r', '').replace('\n', '').replace('\t', '').replace('&nbsp;', '').replace(
                    '<tmdb_data>true</tmdb_data>', '<tmdb_data>all</tmdb_data>')
    ...
    def addDirectory(self, items, queue=False):
        ...
        for i in items:
            try:
                ...
                if mode == 'movies':
                    cm.append(
                        (control.lang(30711).encode('utf-8'), 'RunPlugin(%s?action=addView&content=movies)' % sysaddon))
                elif mode == 'tvshows':
...
class resolver:
    ...
    def process(self, url, direct=True):
        ...
        try:
            ...
            from resources.lib.stemodules import sources
        ...
        try:
            ...
            from resources.lib.stemodules import directstream
        ...
        try:
            ...
            from resources.lib.stemodules import filmon
