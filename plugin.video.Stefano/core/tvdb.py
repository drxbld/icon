...
DICT_MPAA = {'TV-Y': 'Pubblico pre-infantile: minori di 6 anni', 'TV-Y7': 'Pubblico infantile: da 7 anni',
             'TV-G': 'Pubblico generale: senza supervisione', 'TV-PG': 'Pubblico generale: supervisione famiglia',
             'TV-14': 'Maggiori di 14 anni', 'TV-MA': 'Maggiori di 17 anni'}
# Traducciones - Fin
...
def find_and_set_infoLabels(item):
    ...
    if not item.contentSeason:
        p_dialog.update(50, "Cercando informazioni della serie", "Ottenendo risultati...")
    results, info_load = otvdb_global.get_list_results()

    if not item.contentSeason:
        p_dialog.update(100, "Cercando informazioni della serie", "Trovate %s possibili corrispondenze" % len(results))
        p_dialog.close()
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs.get('tvdb_id', ''):
            ...
            if not self.list_results and config.get_setting("tvdb_retry_eng", "biblioteca"):
                from platformcode import platformtools
                platformtools.dialog_notification("Nessun risultato per la lingua '%s'" % DEFAULT_LANG,
                                                  "Cerca in 'en'")
