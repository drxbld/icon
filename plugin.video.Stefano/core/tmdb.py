...
otmdb_global = None
tmdb_auth_key='6a5be4999abf74eba1f9a8311294c267'
...
class Tmdb(object):
    ...
    def rellenar_dic_generos(cls, tipo='movie', idioma='it'):
        ...
        if tipo not in cls.dic_generos[idioma]:
            ...
            url = ('http://api.themoviedb.org/3/genre/%s/list?api_key=%s&language=%s'
                   % (tipo, tmdb_auth_key, idioma))
            try:
    ...
    def __by_id(self, source='tmdb'):
        ...
        if self.busqueda_id:
            if source == "tmdb":
                ...
                #   &append_to_response=images,videos,external_ids,credits&include_image_language=es,null
                url = ('http://api.themoviedb.org/3/%s/%s?api_key=%s&language=%s'
                       '&append_to_response=images,videos,external_ids,credits&include_image_language=%s,null' %
                       (self.busqueda_tipo, self.busqueda_id,tmdb_auth_key, self.busqueda_idioma, self.busqueda_idioma))
                ...
                url = ('http://api.themoviedb.org/3/find/%s?external_source=%s&api_key=%s'
                       '&language=%s' % (self.busqueda_id, source, tmdb_auth_key, self.busqueda_idioma))
        ...
        if resultado and not "status_code" in resultado:
            ...
            #logger.debug(msg) #commented out because filling log
    ...
    def __search(self, index_results=0, page=1):
        ...
        if self.busqueda_texto:
            ...
            url = ('http://api.themoviedb.org/3/search/%s?api_key=%s&query=%s&language=%s'
                   '&include_adult=%s&page=%s' % (self.busqueda_tipo, tmdb_auth_key, self.busqueda_texto.replace(' ', '%20'),
        ...
        if results:
            ...
            #logger.error(msg) #commented out because filling log
            return 0
    ...
    def __discover(self, index_results=0):
        ...
        if type_search:
            ...
            url = ('http://api.themoviedb.org/3/%s?api_key=%s&%s'
                  % (type_search,tmdb_auth_key, "&".join(params)))
    ...
    def get_sinopsis(self, idioma_alternativo=""):
        ...
        if 'id' in self.result:
            ...
            if ret == "" and str(idioma_alternativo).lower() != 'none':
                ...
                url = ('http://api.themoviedb.org/3/%s/%s?api_key=%s&language=%s' %
                       (self.busqueda_tipo, self.busqueda_id, tmdb_auth_key, self.busqueda_idioma))
                try:
                    resultado = jsontools.load_json(scrapertools.downloadpageWithoutCookies(url))
                except:
                    pass
    ...
    def get_temporada(self, numtemporada=1):
        ...
        if not self.temporada.get(numtemporada,{}):
            ...
            url = "http://api.themoviedb.org/3/tv/%s/season/%s?api_key=%s&language=%s" \
                  "&append_to_response=credits" % (self.result["id"], numtemporada,tmdb_auth_key, self.busqueda_idioma)
    ...
    def get_videos(self):
        ...
        if self.result['id']:
            if self.result['videos']:
                ...
                url = "http://api.themoviedb.org/3/%s/%s/videos?api_key=%s&language=%s" \
                      % (self.busqueda_tipo, self.result['id'],tmdb_auth_key, self.busqueda_idioma)
                try:
            ...
            if self.busqueda_idioma != 'en':
                url = "http://api.themoviedb.org/3/%s/%s/videos?api_key=%s" \
                      % (self.busqueda_tipo, self.result['id'],tmdb_auth_key)
                try:
                    dict_videos = jsontools.load_json(scrapertools.downloadpageWithoutCookies(url))
                except:
                    pass
...
def infoSod(item, tipo="movie"):
    ...
    if config.get_setting("scrapedtmdb")==False:
        return item
    logger.info("Stefano.core.tmdb infoSod")
