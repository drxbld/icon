...
addon_name = "plugin://plugin.video.Stefano/"
...
def save_library_movie(item):
    ...
    nfo_path = filetools.join(path, "%s [%s].sod" % (base_name, _id))
...
def save_library_tvshow(item, episodelist):
    ...
    for c in subcarpetas:
        code = scrapertools.find_single_match(c, '\[(.*?)\]')
        if code and code in item.infoLabels['code']:
            c = c.decode(sys.getfilesystemencoding())
            path = c.encode("utf8")
            _id = code
            break
    ...
    if not path:
        ...
        path = filetools.join(TVSHOWS_PATH, path)

    tvshow_path = filetools.join(path, "tvshow.sod")
...
def save_library_episodes(path, episodelist, serie, silent=False, overwrite=True):
    ...
    for e in episodelist:
        if config.get_setting("filtersub", "biblioteca") and "SUB ITA" in e.title:
            continue
        try:
    ...
    for i, e in enumerate(scraper.sort_episode_list(new_episodelist)):
        ...
        nfo_path = filetools.join(path, "%s.sod" % season_episode)
    ...
    if news_in_playcounts:
        ...
        try:
            ...
            logger.error("Error al actualizar tvshow.sod")
            fallidos = -1
        else:
