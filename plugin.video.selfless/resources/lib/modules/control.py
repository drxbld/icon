...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.selflesslive.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.selflesslive.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.selflesslive.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.selflesslive.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.selflesslive.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.selflesslive.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
