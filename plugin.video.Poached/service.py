...
def syncTraktLibrary():
    control.execute(
        'RunPlugin(plugin://%s)' % 'plugin.video.Poached/?action=tvshowsToLibrarySilent&url=traktcollection')
    control.execute(
        'RunPlugin(plugin://%s)' % 'plugin.video.Poached/?action=moviesToLibrarySilent&url=traktcollection')
...
try:
    ...
    log_utils.log('######################### Poached ############################', log_utils.LOGNOTICE)
    log_utils.log('####### CURRENT Poached VERSIONS REPORT ######################', log_utils.LOGNOTICE)
    log_utils.log('### ERROR GETTING Poached VERSIONS - NO HELP WILL BE GIVEN AS THIS IS NOT AN OFFICIAL Poached INSTALL. ###', log_utils.LOGNOTICE)
    log_utils.log('###############################################################', log_utils.LOGNOTICE)
