...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.resistance.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.resistance.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.resistance.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.resistance.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.resistance.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.resistance.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
