...
ADDON_DATA = os.path.join(USERDATA_PATH,'script.module.resistance')
...
def get_rd_domains():
    ...
    cache_location = os.path.join(
            xbmc.translatePath(xbmcaddon.Addon("script.module.resistance").getAddonInfo('profile')).decode('utf-8'),
            'url_cache.db')
    try:
