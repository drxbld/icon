...
username     = xbmcaddon.Addon('plugin.video.Stefano').getSetting('Username')
password     = xbmcaddon.Addon('plugin.video.Stefano').getSetting('Password')

live_url     = 'http://stefanoaddon.info/test/IPTV.xml'
vod_url      = 'http://stefanoaddon.info/test/IPTV.xml'
panel_api    = '%s:%s/panel_api.php?username=%s&password=%s'
...
def start(type):
			...
			#tools.addDir('[COLOR white][B]A[/COLOR][COLOR white]rea Adulti Vip[/COLOR][/B]','url',999991,'https://s18.postimg.org/ggshmv5g9/livetv.png',fanart,"Get Access to All of Your Favourite Channels, In Stunning HD")
			#tools.addDir('[COLOR white][B]L[/COLOR][COLOR white]ive Events[/COLOR][/B]','LIVE',999991,'https://s18.postimg.org/ggshmv5g9/livetv.png',fanart,"Containing Live Events, Including: NBA, NFL, NHL, MLB, English Premier League and PPV!")
			#tools.addDir('[COLOR white][B]C[/COLOR][COLOR white]atchup Tv[/COLOR][/B]','url',9999912,'https://s18.postimg.org/wp8pwceah/CATCHUP.png',fanart,"Get Access To Full 7 Days Catchup Tv On A Whole Bunch Of Channels!")#
			if xbmc.getCondVisibility('System.HasAddon(pvr.iptvsimple)'):
				tools.addDir('[COLOR white][B]T[/COLOR][COLOR green]V Guide[/COLOR][/B]','pvr',999997,'http://stefanoaddon.info/immaggini/tvgiude.png',fanart,"Open Kodi's Inbuilt Tv Guide")
			#tools.addDir('[COLOR white][B]O[/COLOR][COLOR white]n Demand[/COLOR][/B]','vod',999993,'https://s18.postimg.org/82cuys4ex/VOD.png',fanart,'')
			
			tools.addDir('[COLOR white][B]E[/COLOR][COLOR green]xtras[/COLOR][/B]','url',9999916,'http://stefanoaddon.info/immaggini/exstra.png',fanart,"Some Extra Features, Inlcuding Football Guides. Setting Tweaks and More")
	
def home():
			
			#tools.addDir('[COLOR white][B]A[/COLOR][COLOR white]rea Adulti Vip[/COLOR][/B]','live',999991,'https://s18.postimg.org/ggshmv5g9/livetv.png',fanart,'')
			#tools.addDir('[COLOR white][B]L[/COLOR][COLOR white]ive Events[/COLOR][/B]','live',999991,'https://s18.postimg.org/ggshmv5g9/livetv.png',fanart,'')
			#tools.addDir('[COLOR white][B]C[/COLOR][COLOR white]atchup Tv[/COLOR][/B]','url',9999912,'https://s18.postimg.org/wp8pwceah/CATCHUP.png',fanart,'')#
			if xbmc.getCondVisibility('System.HasAddon(pvr.iptvsimple)'):
				tools.addDir('[COLOR white][B]T[/COLOR][COLOR green]V Guide[/COLOR][/B]','pvr',999997,'http://stefanoaddon.info/immaggini/tvgiude.png',fanart,'')
			#tools.addDir('[COLOR white][B]O[/COLOR][COLOR white]n Demand[/COLOR][/B]','vod',999993,'https://s18.postimg.org/82cuys4ex/VOD.png',fanart,'')
			
			tools.addDir('[COLOR white][B]E[/COLOR][COLOR green]xtras[/COLOR][/B]','url',9999916,'http://stefanoaddon.info/immaggini/exstra.png',fanart,'')			
...
def addonsettings(url,description):
	...
	if   url =="CC":
		...
		if 'ON' in description:
			xbmcaddon.Addon('plugin.video.Stefano').setSetting('vodsource','false')
			xbmc.executebuiltin('Container.Refresh')
		else:
			xbmcaddon.Addon('plugin.video.Stefano').setSetting('vodsource','true')
			xbmc.executebuiltin('Container.Refresh')
	elif url =="LO":
...
def correctPVR():
	...
	loginurl   = 'https://pastebin.com/raw/9puH34aG'
	EPGurl     = 'http://www.epg-guide.com/it.gz'
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno(user.name,'Vuoi Configurae il PVR TV per gli EPG? ')
		if dialog:
				correctPVR()
				xbmcgui.Dialog().ok(user.name, 'PVR TV Guida Tv Installazione Completa - Riavvia Kodi Per Vedere i Cambiamenti')
def num2day(num):
	if num =="0":
		day = 'monday'
	elif num=="1":
...
def extras():
	...
	tools.addDir('Integrate With PVR TV Guide','tv',9999917,icon,fanart,"Integrate Stefano Premium Live Streams and EPG with Kodi's inbuilt TV Guide.[CR][CR]Allowing A Full, Sleek Looking TV Guide!")
	if xbmcaddon.Addon('plugin.video.Stefano').getSetting('vodsource')=='true':
		VOD = '[B][COLOR lime]ON[/COLOR][/B]'
	else:
		VOD = '[B][COLOR red]OFF[/COLOR][/B]'
		
#	tools.addDir('Show Premium Links in Stefano Movie/Show Sources: %s'%VOD,'VODSOURCE',9999910,icon,fanart,VOD)
	...
	if description=='BBC1 Scotland':
		...
		for a in all_chans:
			...
			if chan in name.lower():
				tools.addDir(name.replace('UK:','[COLOR white][B]UK:[/COLOR][/B]').replace('USA/CA:','[COLOR white][B]USA/CA:[/COLOR][/B]').replace('All','[COLOR white][B]All[/COLOR][/B]').replace('International Sport','[COLOR white][B]INT: [/COLOR][/B]International Sport').replace('Live:','[COLOR white][B]Live:[/COLOR][/B]').replace('TEST','[COLOR white][B]TEST[/COLOR][/B]').replace('Install','[COLOR white][B]Install[/COLOR][/B]').replace('24/7','[COLOR white][B]24/7: [/COLOR][/B]Channels').replace('DE:','[COLOR white][B]DE:[/COLOR][/B]').replace('FR:','[COLOR white][B]FR:[/COLOR][/B]').replace('PL:','[COLOR white][B]PL:[/COLOR][/B]').replace('AR:','[COLOR white][B]AR:[/COLOR][/B]').replace('LIVE:','[COLOR white][B]LIVE:[/COLOR][/B]').replace('ES:','[COLOR white][B]ES:[/COLOR][/B]').replace('IN:','[COLOR white][B]IN:[/COLOR][/B]').replace('PK:','[COLOR white][B]PK:[/COLOR][/B]').replace('NBC Extra Time','[COLOR white][B]NBC:[/COLOR][/B] NBC Extra Time'),play_url+url+'.ts',999994,thumb,fanart,'')
...
def apkdownloads():
	open = requests.get('http://stefanoaddon.info/addon/apks.txt').text
