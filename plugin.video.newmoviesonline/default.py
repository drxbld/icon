...
ADDON_PATH = xbmc.translatePath('special://home/addons/plugin.video.newmoviesonline')# you need to change the folder path here to the name of you plugin folder 
ICON = ADDON_PATH + 'icon.png'
FANART = ADDON_PATH + 'fanart.jpg'
USERDATA_PATH = xbmc.translatePath('special://home/userdata/addon_data')
ADDON_DATA = USERDATA_PATH + '/plugin.video.newmoviesonline/'# you need to change the folder path here to the name of you plugin folder 
...
def Main_Menu():
    url='http://newmoviesonline.tv/'
    html= requests.get(url).content
    block= re.compile('class="genre_text_block"(.+?)<div class="title_block"',re.DOTALL).findall(html)
    match= re.compile('<li class="cat-item.+?href="(.+?)" >(.+?)</a>.+?</li>').findall(str(block))
    for link, name in match:

        Menu(name,link,21,'http://newmoviesonline.tv/wp-content/themes/newmoviesonline/images/logo.png','https://ftp.heanet.ie/mirrors/xbmc/addons/frodo/script.artwork.downloader/fanart.jpg','Enter Here','')

def Second_Menu(url):
    html= requests.get(url).content
    block=re.compile('class="thumbs_container"(.+?)class="pages">.+?<ul>',re.DOTALL).findall(html)
    match= re.compile('class="thumb">.+?href="(.+?)".+?src="(.+?)".+?alt="(.+?)".+?</ul>',re.DOTALL).findall(str(block))
    for link,img,name in match:
        Menu(name,link,22,img,'','Enter Here','')
    
def Third_Menu(url):
    html= requests.get(url).content
    block=re.compile(' class="video_info"(.+?)id="disqus_thread"',re.DOTALL).findall(html)
    match= re.compile('<a href="(.+?)".+?<br/>',re.DOTALL).findall(str(block))
    for link1 in match:
        if '/watch/onlinehd' not in link1:
            link = 'https:'+link1
...
def Menu(name,url,mode,iconimage,fanart,description,extra,showcontext=True,allinfo={}):
        ...
        if showcontext:
            contextMenu = []
            if showcontext == 'fav':
                contextMenu.append(('Remove from newmoviesonline Favorites','XBMC.RunPlugin(%s?mode=12&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('Add to newmoviesonline Favourites','XBMC.RunPlugin(%s?mode=11&name=%s&url=%s&iconimage=%s&fav_mode=%s&fanart=%s&description=%s)'
...
def getFavourites():
    if not os.path.exists(favourites):
        favList = []
        favList.append(('newmoviesonline Favourites Section', '', '', '', '', '', ''))
        a = open(favourites, "w")
        a.write(json.dumps(favList))
        a.close()
    else:
