...
def AUTHENTICATE():
    DATA_SETTINGS_FILE   =  xbmc.translatePath(os.path.join('special://home/userdata/addon_data/plugin.video.sanity/settings.xml'))
	
    if not os.path.exists(DATA_SETTINGS_FILE):
        xbmcgui.Dialog().ok("[B][COLOR blue]Sanity[/COLOR][/B]", "You are required to login to use [B][COLOR blue]Sanity[/COLOR][/B]!", "Please enter your login details at the next prompt.","To sign up visit - [COLOR blue]www.sandmanmedia.xyz/members[/COLOR] - You must be a member of our Telegram group and confirm your email with an admin!")
    ...
    if not base64.b64decode('MQ==') in data:
		...
		dialog.ok('[B][COLOR blue]Sanity[/COLOR][/B]', "[COLOR red][B]Error:[/B][/COLOR] You have not entered a valid E-Mail and Password.")
		plugintools.open_settings_dialog()	
		sys.exit(0)
