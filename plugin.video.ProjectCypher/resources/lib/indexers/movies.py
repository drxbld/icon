...
from resources.lib.addon import utils
...
class movies:
    def __init__(self):
        ...
        self_uservar = control.setting('uservar')
        self_useramount = control.setting('useramount')
        ...
        self.keywords_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie,documentary&num_votes=100,&production_status=released&keywords=%s&sort=%s,asc&count=%s&start=1' % ( '%s', self_uservar, self_useramount)
        self.persons_link = 'http://www.imdb.com/search/name?count=100&name='
        self.personlist_link = 'http://www.imdb.com/search/name?count=100&gender=male,female'
        self.popular_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie&num_votes=1000,&production_status=released&groups=top_1000&sort=moviemeter,asc&count=%s&start=1' % (self_useramount)
        self.views_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie&num_votes=1000,&production_status=released&sort=num_votes,desc&count=%s&start=1' % (self_useramount)
        self.featured_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie&num_votes=1000,&production_status=released&release_date=date[365],date[60]&sort=moviemeter,asc&count=%s&start=1' % (self_useramount)
        self.person_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie&production_status=released&role=%s&sort=year,desc&count=%s&start=1' % ('%s',self_useramount)
        self.genre_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie,documentary&num_votes=100,&release_date=,date[0]&genres=%s&sort=%s,asc&count=%s&start=1' % ( '%s', self_uservar, self_useramount)
        self.keyword_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie,documentary&num_votes=100,&release_date=,date[0]&keywords=%s&sort=%s,asc&count=%s&start=1' % ( '%s', self_uservar, self_useramount)
        #self.language_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie&num_votes=100,&production_status=released&primary_language=%s&sort=moviemeter,asc&count=%s&start=1' % (self_useramount)
        self.certification_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie&num_votes=100,&production_status=released&certificates=us:%s&sort=%s,asc&count=%s&start=1'% ( '%s', self_uservar, self_useramount)
        self.year_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie&num_votes=100,&production_status=released&year=%s,%s&sort=%s,asc&count=%s&start=1' % ( '%s','%s', self_uservar, self_useramount)
        self.boxoffice_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie&production_status=released&sort=boxoffice_gross_us,desc&count=%s&start=1' % (self_useramount)
        self.oscars_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie&production_status=released&groups=oscar_best_picture_winners&sort=%s,asc&count=40&start=1' % ( self_uservar)
        date = 'date%5B365%5D,date%5B0%5D'
        self.theaters_link = 'http://www.imdb.com/search/title?title_type=feature&num_votes=1000,&release_date=%s&count=%s&start=1&sort=release_date,desc'  % (date, self_useramount)
    ...
    def keywords(self):
        keywords = [
            ...
            ('Characters point of view camerashot','characters-point-of-view-camera-shot',True),
            ...
            ('Cyber punk','cyberpunk',True),
            ('Dark hero','dark-hero',True),
            ('Deusex machina','deus-ex-machina',True),
            ...
            ('Husband wifer elationship','husband-wife-relationship',True),
            ...
            ('Neonoir','neo-noir',True),
            ('Neorealism','neorealism',True),
            ('Ninja','ninja',True),
            ('Noback ground score','no-background-score',True),
            ('Nomusic','no-music',True),
            ...
            ('Organize dcrime','organized-crime',True),
            ...
            ('Swordandsandal','sword-and-sandal',True),
        ...
        for i in keywords: self.list.append(
            {    
                ...
                'url': self.keywords_link % i[1] if i[2] else self.keyword_link % i[1],
                'image': 'keywords.png',
                'action': 'movies'
            })
            #url = self.search_link + urllib.quote_plus(q)
            #url = '%s&sort=%s,asc&count=40&start=1' % (keywords_link, uv)
            #control.execute('Container.Update(%s)' % url)
    ...
    def worker(self, level=1):
        ...
        self.fanart_tv_headers = {'api-key': 'ZjU4Y2EwOThmOTlhYWQzMDU1NzE0YzFkOTNhYjkzYTc='.decode('base64')}
    ...
    def movieDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                item = control.item(label=' %s ' % (label))
