...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.no-name.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.no-name.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.no-name.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.no-name.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.no-name.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.no-name.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
...
def current():
        if uv == 'moviemeter':
            ...
            return 'US Box Office by Gross'
        elif uv == 'year':
            return 'By Year'
        elif uv == 'release_date':
