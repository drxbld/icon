        ...
        if endpoint:
            if endpoint == 'replays':
                if link:
                    ...
                    replays.get_races(page)
            else:
                raise ValueError('Invalid paramstring: {0}!'.format(paramstring))
        else:
            raise ValueError('Invalid paramstring: {0}!'.format(paramstring))
    else:
        list_item = xbmcgui.ListItem(label='Full-Race Replays')
        list_item.setArt({'fanart':fanart,'thumb':icon,'poster':icon})
        list_item.setInfo('video',{'plot':'Full-Race Replays section for Indycar, Formula 1, Moto GP & NASCAR.\n\n[COLOR yellow]Note:[/COLOR] Pin Generation is required to use this section.','title':'Full-Race Replays'})
        url = get_url(sport='motorsports', endpoint='replays')
