...
fanart = xbmc.translatePath(os.path.join(path, "resources/wwe/background.jpg"))
h = HTMLParser()
wp = 'http://fullmatchtv.com/index.php/wp-json/wp/v2/posts?categories=989'
...
def get_games(page=1):
    ...
    if resp.status_code != 400:
        ...
        for game in resp.json():
            ...
            if 'wwe' not in title.lower():
                continue
            description = re.compile('<h2>(.+?)</h2>').findall(game['content']['rendered'])[0]
            description = description.replace(' &#8211; ','\n').replace('Play Off ','Play Off\n')
            description += '\n\n[COLOR yellow]Note:[/COLOR] Due to the nature of wrestling\'s strict copyright policies, the further you go back the more likely the airings are no longer available/have been deleted. Especially for PPV events. It\'s best to watch a show the day after it airs to increase the chances of catching the file before it\'s deleted.'
            ...
            for s, source in enumerate(sources):
                ...
                list_item.setArt({'fanart':fanart,'thumb':icon,'poster':icon})
                url = get_url(sport='wwe', endpoint='replays', link=source, title=title)
                xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
                currentCount += 1
        if currentCount > 0:
            list_item = xbmcgui.ListItem(label='[B]<<< EARLIER SHOWS[/B]')
            list_item.setArt({'fanart':fanart,'thumb':icon,'poster':icon})
            url = get_url(sport='wwe', endpoint='replays', page=int(page)+1)
            xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
