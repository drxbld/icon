...
h = HTMLParser()
wp = 'http://fullmatchtv.com/index.php/wp-json/wp/v2/posts?categories=3'
...
def show_qualities(link, title):
    ...
    for quality, link in quals:
        ...
        #url = get_url(sport='nba', endpoint='replays', link=link, title=title)
    ...
    if resp.status_code != 400: 
        ...
        for game in resp.json():
            ...
            for s, source in enumerate(sources):
                ...
                list_item.setArt({'fanart':fanart,'thumb':icon,'poster':icon})
                url = get_url(sport='nba', endpoint='replays', link=source, title=title)
                xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
                currentCount += 1
        if currentCount > 0:
            list_item = xbmcgui.ListItem(label='[B]<<< EARLIER GAMES[/B]')
            list_item.setArt({'fanart':fanart,'thumb':icon,'poster':icon})
            url = get_url(sport='nba', endpoint='replays', page=int(page)+1)
            xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
