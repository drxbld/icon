...
fanart = xbmc.translatePath(os.path.join(path, "resources/rugby/background.jpg"))
h = HTMLParser()
wp = 'http://fullmatchtv.com/index.php/wp-json/wp/v2/posts?categories=928'
    ...
    if resp.status_code != 400:
        ...
        for game in resp.json():
            ...
            for s, source in enumerate(sources):
                ...
                list_item.setArt({'fanart':fanart,'thumb':icon,'poster':icon})
                url = get_url(sport='rugby', endpoint='replays', link=source, title=title)
                xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
                currentCount += 1
        if currentCount > 0:
            list_item = xbmcgui.ListItem(label='[B]<<< EARLIER GAMES[/B]')
            list_item.setArt({'fanart':fanart,'thumb':icon,'poster':icon})
            url = get_url(sport='rugby', endpoint='replays', page=int(page)+1)
            xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
