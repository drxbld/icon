...
"""
    //dreadnought Add-on//
    Updated for dreadnought Add-on
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.dreadnought.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.dreadnought.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.dreadnought.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.dreadnought.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.dreadnought.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.dreadnought.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
