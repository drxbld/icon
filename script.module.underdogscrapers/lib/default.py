...
tmdb_test = str(xbmcaddon.Addon('script.module.underdogscrapers').getSetting("tmdb_test"))

ADDON_PATH = xbmc.translatePath('special://home/addons/script.module.underdogscrapers/')
ICON = ADDON_PATH + 'icon.png'
FANART = ADDON_PATH + 'fanart.jpg'

USERDATA_PATH = xbmc.translatePath('special://home/userdata/addon_data')
ADDON_DATA = os.path.join(USERDATA_PATH,'script.module.underdogscrapers')
...
scrapers_path = xbmc.translatePath('special://home/addons/script.module.underdogscrapers/lib/underdogscrapers/scraperplugins')
...
if mode == "DisableAll":
    scrapers = sorted(
        underdogscrapers.relevant_scrapers(include_disabled=True), key=lambda x: x.name.lower())
    for scraper in scrapers:
        key = "%s_enabled" % scraper.name
        xbmcaddon.Addon('script.module.underdogscrapers').setSetting(key, "false")
    sys.exit()
elif mode == "EnableAll":
    scrapers = sorted(
        underdogscrapers.relevant_scrapers(include_disabled=True), key=lambda x: x.name.lower())
    for scraper in scrapers:
        key = "%s_enabled" % scraper.name
        xbmcaddon.Addon('script.module.underdogscrapers').setSetting(key, "true")
    sys.exit()
elif mode == "Deletelog":
    from underdogscrapers.common import Del_LOG
    Del_LOG()
    sys.exit()
...
def get_scraper_results():
	try:
		...
		if not os.path.exists(scraper_results_path):
			...
			get_info = re.findall('<.+?underdogscraper: (.+?)\n.+?Tested with: (.+?)\n.+?Links returned: (.+?)\n.+?Time to Complete:(.+?)\n',str(Open),re.DOTALL)
		...
		if results_type == 0:
			Open = open(scraper_results_path).read()
			get_line = re.findall('(.+?)\n',Open,re.DOTALL)
			dialog.textviewer("underdogscrapers Testing Mode", '\n'.join(str(p) for p in get_line) )
		elif results_type == 1:
...
def test():
	pDialog = xbmcgui.DialogProgress()
	if dialog.yesno("underdogscrapers Testing Mode", 'Clear Scraper Log?'):
		clear_scraper_log()
	if dialog.yesno("underdogscrapers Testing Mode", 'Clear cache?'):
		underdogscrapers.clear_cache()
	if tmdb_test == '':
...
def tmdb_movies(html):
	count = 0
	pDialog.create('underdogscrapers Testing mode active', 'please wait')
	...
	for url, name in match:
		...
		for title,year in match2:
			title = title[:-1]
			movie_links_scraper = underdogscrapers.scrape_movie(title, year, '')
...
def single_test(count, index):
	if count==5:
		...
		pDialog.create('underdogscrapers Testing mode active', 'please wait')
		if dialog.yesno("underdogscrapers Testing Mode", 'Run next Movie?',Movies['title']+' ('+Movies['year']+')'):
			movie_links_scraper = underdogscrapers.scrape_movie(Movies['title'], Movies['year'], Movies['imdb'])
			movie_links_scraper = movie_links_scraper()
		...
		Scrapers_Run = 0
		if dialog.yesno("underdogscrapers Testing Mode", 'Would you like to run a tv show?',
		tv_shows['title']+' ('+tv_shows['year']+') S'+tv_shows['season']+'E'+tv_shows['episode']):
			episode_links_scraper = underdogscrapers.scrape_episode(tv_shows['title'], tv_shows['show_year'], tv_shows['year'], tv_shows['season'], tv_shows['episode'], tv_shows['imdb'],'')
...
def full_test():
	...
	for item in movies:
		...
		if pDialog.iscanceled():
			break
		movie_links_scraper = underdogscrapers.scrape_movie(item['title'], item['year'], item['imdb'])
		movie_links_scraper = movie_links_scraper()
	...
	for item in shows:
		...
		if pDialog.iscanceled():
			break
		episode_links_scraper = underdogscrapers.scrape_episode(item['title'], item['show_year'], item['year'], item['season'], item['episode'], item['imdb'],'')
		episode_links_scraper = episode_links_scraper()
