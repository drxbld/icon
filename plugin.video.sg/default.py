...
SGTV = base64.b64decode('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3NpbW9uMjUxMS9zZ3R2L21hc3Rlci9zZy54bWw=')


addon = xbmcaddon.Addon('plugin.video.sg')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.sg]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification([COLOR white][B]SGTV[/B][/COLOR],We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification([COLOR white][B]SGTV[/B][/COLOR],We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR white][B]SGTV[/B][/COLOR],New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('[COLOR white][B]SGTV[/B][/COLOR] Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
            ...
            if k in regexs:
                ...
                if m['page'] and m['page'] in cachedPages and not 'ignorecache' in m and forCookieJarOnly==False :
                    ...
                    if m['page'] and  not m['page']=='' and  m['page'].startswith('http'):
                        ...
                        req.add_header('User-Agent', 'MyAgent')
                        if 'refer' in m:
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR white][B]SGTV[/B][/COLOR],Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(sgtv,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('[COLOR white][B]SGTV[/B][/COLOR]','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('[COLOR white][B]SGTV[/B][/COLOR]', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from [COLOR white][B]SGTV[/B][/COLOR] Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('Add to [COLOR white][B]SGTV[/B][/COLOR] Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        headers = dict({'User-Agent':'MyAgent','Referer': 'http://joker.org/','Accept-Encoding':'gzip, deflate','Content-Type': 'application/json;charset=utf-8','Accept': 'application/json, text/plain, */*'})
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification([COLOR white][B]SGTV[/B][/COLOR],No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from [COLOR white][B]SGTV[/B][/COLOR] Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to [COLOR white][B]SGTV[/B][/COLOR] Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR white][B]SGTV[/B][/COLOR],Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification([COLOR white][B]SGTV[/B][/COLOR],Please [COLOR blue]install Youtube-dl[/COLOR] module ,10000,"")")
