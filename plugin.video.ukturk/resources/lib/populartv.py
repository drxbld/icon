...
def TVShows(url):
	...
	stringlist=''
	link = open_url(url)
	c = re.findall('<div class="movie-list">(.*?)</a>',link,flags=re.DOTALL)
	for data in c:
		pattern = '''<a\s+href="(.*?)".+?img\s+src="(.*?)"\s+alt="(.*?)"'''
		shows = re.findall(pattern,data)
		for link,icon,name in shows:
			string='<start>'+name+'<sep>'+link+'<sep>'+icon+'<end>'
