...
def Shows(url):
    stringlist=''
    link=open_url(url)
    match=re.compile('<div class="programs_item col-lg-3 col-md-3 col-sm-4 col-xs-4 col-xxs-6">.+?<a href="(.+?)">.+?<img class="img-responsive" src="(.+?)" alt="(.+?)" />',re.DOTALL).findall(link)
...
def Stream(url):
    link=open_url(url)
    host_link=re.compile('url: "(.+?)?=.+?"').findall(link)
    print host_link
