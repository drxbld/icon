...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.Medusa.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.Medusa.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.Medusa.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.Medusa.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.Medusa.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.Medusa.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
