...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.watch_dogs.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.watch_dogs.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.watch_dogs.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.watch_dogs.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.watch_dogs.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.watch_dogs.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
