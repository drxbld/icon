...
def addonFanart():
    theme = appearance() ; art = artPath()
    if not (art == None and theme in ['-', '']): return os.path.join(art, 'fanart.gif')
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.Legendary.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.Legendary.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.Legendary.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.Legendary.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.Legendary.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.Legendary.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
