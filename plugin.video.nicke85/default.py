...
import urllib, urlparse, sys, xbmcplugin ,xbmcgui, xbmcaddon, xbmc, os, json, hashlib, base64
...
playlistsFile = os.path.join(addonDir, "default.png")
...
def Categories():
	...
	for item in chList:
		mode = 1 if '.plx' in item["url"][::-1].decode('base64').replace("83eT","Ppbi").replace("5ePP","0ZWJ").decode('base64').replace("7RnT","6Fqn") else 2
		name = common.GetEncodeString(item["name"])
		image = item.get('image', '')
		logos = item.get('logos', '')
		cacheMin = item.get('cache', '0')
		if item["url"][::-1].decode('base64').replace("83eT","Ppbi").replace("5ePP","0ZWJ").decode('base64').replace("7RnT","6Fqn").startswith('http'):
			cacheList.append(hashlib.md5(item["url"][::-1].decode('base64').replace("83eT","Ppbi").replace("5ePP","0ZWJ").decode('base64').replace("7RnT","6Fqn").encode("utf-8")).hexdigest())
		m3uCategory(item["url"][::-1].decode('base64').replace("83eT","Ppbi").replace("5ePP","0ZWJ").decode('base64').replace("7RnT","6Fqn").encode("utf-8"), logos.encode("utf-8"), cacheMin, index)
		i += 1
...
def AddNewList():
	...
	chList = common.ReadList(playlistsFile)
	for item in chList:
		if item["url"][::-1].decode('base64').replace("83eT","Ppbi").replace("5ePP","0ZWJ").decode('base64').replace("7RnT","6Fqn").lower() == listUrl.lower():
	...
	for channels in groupChannels:
		...
		for channel in chs:
			...
			if logos is not None and logos != ''  and image != "" and not image.startswith('http'):
				image = logos + image
			AddDir(name.replace("  -  ","  •  "), chUrl, 3, image, index=-1, isFolder=False, IsPlayable=True)
...
def AddFavorites(url, iconimage, name):
	...
	for item in favList:
		if item["url"][::-1].decode('base64').replace("83eT","Ppbi").replace("5ePP","0ZWJ").decode('base64').replace("7RnT","6Fqn").lower() == url.decode("utf-8").lower():
...
def AddNewFavorite():
	...
	favList = common.ReadList(favoritesFile)
	for item in favList:
		if item["url"][::-1].decode('base64').replace("83eT","Ppbi").replace("5ePP","0ZWJ").decode('base64').replace("7RnT","6Fqn").lower() == chUrl.decode("utf-8").lower():
