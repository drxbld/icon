...
class source:
    ...
    def get_sources(self, url, hosthdDict, hostDict, locDict):
        control.log('###DIZIGOLD - SOUR %s' % url)
        ...
        try:
            ...
            post = {'id': post, 'tip':'view','dil':'en'}

            result = client.request(query, post=post, headers=self.headers)
            control.log('###DIZIGOLD - SOUR post res %s' % result)

            result = json.loads(result)
            result = result['data']
            control.log('moja data %s' % result)

            result = re.compile('iframe.*?src="([^"]+)"').findall(result)[0]
            control.log('###DIZIGOLD - SOUR post res2 %s' % result)

            result = client.request(result, headers={'Referer':url})
            control.log('###DIZIGOLD - SOUR post res3 %s' % result)
