...
class source:
		...
		try:
			...
			try: # Page 1
				...
				for url,filename in match: 
					...
					match = re.compile('<strong>Infohash :</strong> <span>(.+?)</span>').findall(r)
					for hash in match:
						quality = quality
						hash = hash
	...
	def resolve(self, url):
		...
		for url,filename,id in match:
			r = client.request(url)
			match = re.compile('href="(.+?)" onclick="javascript\: count\(this\)\;"><span class="icon"><i class="flaticon-magnet">').findall(r)
