...
class source:
	...
	def tvshow(self, imdb, tvdb, tvshowtitle, localtvshowtitle, aliases, year):
		if xbmc.getCondVisibility('System.HasAddon(script.realdebrid.mod)'):
			try:
				...
				#r = r.replace("&#039;","'")
				match = re.compile('<option value="(.+?)">' + tvshowtitle + '</option>').findall(r)
				for url in match: 
					return url
			except:
				return
	...
	def episode(self, url, imdb, tvdb, title, premiered, season, episode):
		try:
			...
			r = client.request(url)
			#xbmcgui.Dialog().textviewer('testing',r)
			#match = re.compile('New episode\: .+? S' + season + 'E' + episode + ' .+?Link\: <a href="magnet\:\?xt=urn\:btih\:(.+?)\&.+?">').findall(r)
			match = re.compile('<tv\:raw_title>.+? S' + season + 'E' + episode + '.+?</tv\:raw_title><tv\:info_hash>(.+?)</tv\:info_hash>').findall(r)
			for url in match:
				return url
		except:
			return
