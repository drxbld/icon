...
class source:
	def __init__(self):
		...
		self.base_link = 'https://limetorrents.cc'
	...
	def sources(self, url, hostDict, hostprDict):
		try:
			...
			try: # Page 1
				...
				match = re.compile('<div class="tt-name"><a href="http://itorrents\.org/torrent/(.+?)\.torrent\?title=.+?" rel="nofollow" class="csprite_dl14"></a><a href="(.+?)">(.+?)</a>').findall(r)
				for hash,url,info in match: 
					torrenturl = 'https://www.limetorrents.cc%s' % (url)
	...
	def resolve(self, url):
		...
		for url,filename,id in match:
			r = client.request(url)
			match = re.compile('<a href="(.+?)" rel="nofollow">Magnet Link</a>').findall(r)
