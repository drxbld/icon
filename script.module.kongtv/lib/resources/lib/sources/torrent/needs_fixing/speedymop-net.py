...
class source:
	def __init__(self):
		...
		self.base_link = 'http://speedymop.net'
		self.search_link = '/search?q=%s&category[]=1&country[]=2&country[]=226&country[]=1&page=%s'
	...
	def sources(self, url, hostDict, hostprDict):
		try:
			...
			try: # Page 1
				...
				match = re.compile('<a href="magnet\:\?xt=urn\:btih\:(.+?)&').findall(r)
				for url in match: 
					torrenturl = url
