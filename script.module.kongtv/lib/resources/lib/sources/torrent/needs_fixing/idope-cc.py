...
class source:
	def __init__(self):
		...
		self.base_link = 'https://idope.cc'
		self.search_link = '/torrent-list/%s/?p=%s'
	...
	def sources(self, url, hostDict, hostprDict):
		try:
			...
			try: # Page 1
				...
				match = re.compile('<div id="hideinfohash1" class="hideinfohash">(.+?)</div>\n								<div id="hidename1" class="hideinfohash">(.+?)</div>').findall(r)
				for url,filename in match: 
					torrenturl = 'https://idope.se/torrent/%s' % (url)
	...
	def resolve(self, url):
		...
		for url,filename,id in match:
			r = client.request(url)
			match = re.compile('<a id="mangetinfo" href="(.+?)">').findall(r)
